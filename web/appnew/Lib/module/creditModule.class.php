...
class creditModule extends SiteBaseModule {
    ...
    function index() {
    	...
    	//来来1号店
    	$urlre = SITE_DOMAIN.APP_ROOT.'/index.php?ctl=collocation&act=SetRealName&user_id='.$GLOBALS['user_info']['id'];
    	$GLOBALS['tmpl']->assign('urlre',$urlre);
    ...
    function credit_save(){
    	...
    	$GLOBALS['db']->autoExecute(DB_PREFIX."user_credit_file",$data,$mode,$condition);
     	...
     	//file_put_contents("data3.txt",$GLOBALS['user_info']['id']);   
    	//来来1号店
    	$GLOBALS['db']->query("update ".DB_PREFIX."user set idcardpassed = 2 where id =". $GLOBALS['user_info']['id'] );
