...
class indexModule extends SiteBaseModule
{
	public function index()
	{
		...
		{	
			...
			//输出公告
			$notice_list = get_notice(6);
			$GLOBALS['tmpl']->assign("notice_list",$notice_list);	
			
			//输出标的预告
			$art_id =  $GLOBALS['db']->getOne("SELECT id FROM ".DB_PREFIX."article_cate where title='标的预告'");
		
			if($art_id > 0){
				$compnay_active_list  = get_article_list(3,$art_id);
			...
			$GLOBALS['tmpl']->assign("show_site_titile",1);
                        	//首页新增运营时间
            $yy_time_start = 1433091661; //起始时间  1450852650  1451577600
            $now_time = time();
            $yytime = array();
            ...
            //网站收益
            $virtual_money_1 = strip_tags(number_format(floatval($GLOBALS['db_conf']['VIRTUAL_MONEY_1'] + $stats['total_load']) , 2,".",",")); //虚拟的累计成交额;
            $virtual_money_2 = strip_tags(number_format(floatval($GLOBALS['db_conf']['VIRTUAL_MONEY_2'] + $stats['total_rate']) , 2,".",",")); //虚拟的累计创造收益;
            $GLOBALS['tmpl']->assign("virtual_money_1", $virtual_money_1);
            $GLOBALS['tmpl']->assign("virtual_money_2", $virtual_money_2);
		}
