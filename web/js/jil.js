// JavaScript Document
 ...
 function BindCity(City){
    ...
    city.innerHTML = "";
    city.options[0] = new Option(opt0City,"");
    ...
    for(var i=0; i<cityCount; i++){
        if(cityCount == 1 && i == 0){
            city.options[i+1] = new Option(china[selectProvinceIndex][i],china[selectProvinceIndex][i]);
            i = 1;
        }
        else if(cityCount > 1 && i == 0){
            i = 1;
            city.options[i] = new Option(china[selectProvinceIndex][i],china[selectProvinceIndex][i]);
        }
        else{
            city.options[i] = new Option(china[selectProvinceIndex][i],china[selectProvinceIndex][i]);
        }
