...
<div id="mainContainer">
    ...
    <fieldset>
        ...
        <div>本页面仅在测试环境使用，可根据提示输入参数测试创建会员接口。demo使用时需要开启phpcurl扩展和openssl！，
            ...
            <h3 style="color: red">
                1.此接口用户获取用户在新浪绑定的手机号，邮箱，实名证件信息。
            </h3>
        </div>
    </fieldset>
    <br> <br>
    <fieldset>
        ...
        <input style="height: 40px; width: 150px;"
               type="button" value="查询认证信息" id="test_weibopay">&nbsp;<input
        ...
        <form id="request_form" name="request_form">
            <table style="text-align: left; width: 100%; font-size: 12px;"
                ...
                <tr>
                    <td style="vertical-align: top; width: 25%;">是否掩码： <br>
                    </td>
                    <td style="vertical-align: top; width: 50%"><input
                            style="width: 100%;" id="is_mask" name="is_mask"
                            value="N"> <br></td>
                    <td style="vertical-align: top; width: 25%;">返回认证内容是否掩码显示， 如手机号掩码为13****8。Y-是；N-否。为空默认为Y。<br>
                    </td>
                </tr>
...
<script>
    ...
    function checkForm() {
        ...
        var is_mask = $("#is_mask").val();
        if (typeof (is_mask) != "undefined" && is_mask) {
            params += '&is_mask=' + is_mask;
        }
