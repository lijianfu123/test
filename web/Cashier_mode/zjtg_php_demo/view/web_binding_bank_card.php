...
<div id="mainContainer">
    ...
    <fieldset>
        <legend> 介绍</legend>
        <div>本页面仅在测试环境使用，可根据提示输入参数测试我的银行卡接口。demo使用时需要开启phpcurl,openssl扩展！</div>
    </fieldset>
    <br> <br>
    <fieldset>
        <legend> 调用参数</legend>
        <input style="height: 40px; width: 150px;"
               type="button" value="查看我的银行卡" id="test_weibopay">&nbsp;<input
        ...
        <form id="request_form" name="request_form">
            <table style="text-align: left; width: 100%; font-size: 12px;"
                ...
                <tr>
                    ...
                    <td style="vertical-align: top; width: 50%">
                        <select name="service" id="service">
                            <option value="web_binding_bank_card">查看我的银行卡</option>
                        </select>
                        <br></td>
                    <td style="vertical-align: top; width: 25%;">接口名称 非空
                        web_binding_bank_card查看我的银行卡
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <td style="vertical-align: top; width: 25%;">绑定标识： <br>
                    </td>
                    <td style="vertical-align: top; width: 50%">

                        <select id="could_bind" name="could_bind">
                            <option value="Y">Y</option>
                            <option value="N">N</option>
                        </select>
                        <br></td>
                    <td style="vertical-align: top; width: 25%;">是否允许用户绑定银行卡。Y：允许；N：不允许。默认为Y（允许绑定）</td>
                </tr>
                <tr>
                    <td style="vertical-align: top; width: 25%;">删除标识： <br>
                    </td>
                    <td style="vertical-align: top; width: 50%">

                        <select id="could_unbind" name="could_unbind">
                            <option value="Y">Y</option>
                            <option value="N">N</option>
                        </select>
                        <br></td>
...
<script>
        ...
        if (p) {
            $.post('../controller/controller.php', p, function (data) {
                if (data.response_code == "APPLY_SUCCESS") {
                    ...
                    var resu = "response_message:" + data.response_message + "response_code:" + data.response_code;
                    $("#result").val(resu);
                }
            }, 'json');
        }
    ...
    function checkForm() {
        ...
        var could_bind = $("#could_bind").val();
        if (typeof (could_bind) != "undefined" && could_bind) {
            params += '&could_bind=' + could_bind;
        } else {
            $("#error").html("请输入绑定标识");
            return false;
        }
        var could_unbind = $("#could_unbind").val();
        if (typeof (could_unbind) != "undefined" && could_unbind) {
            params += '&could_unbind=' + could_unbind;
        } else {
            $("#error").html("请输入删除标识");
            return false;
        }
