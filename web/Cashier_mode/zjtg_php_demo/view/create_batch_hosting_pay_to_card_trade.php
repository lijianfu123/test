...
<div id="mainContainer">
    ...
    <fieldset>
        ...
        <div>本页面仅在测试环境使用，可根据提示输入参数测试创建会员接口。demo使用时需要开启phpcurl扩展和openssl！，
            ...
            <h3 style="color: red">网银支付联调环境仅支持TESTBANK ，快捷和绑定支付仅支持存钱罐账户的托管充值交易，测试限额 basic账户限额1-119999，SAVING_POT：1-4999
    ...
    <fieldset>
        ...
        <input style="height: 40px; width: 150px;"
               type="button" value="创建批量代付到提现卡" id="test_weibopay">&nbsp;<input
            ...
            <table style="text-align: left; width: 100%; font-size: 12px;"
                ...
                <tr>
                    ...
                    <td style="vertical-align: top; width: 50%"><input
                            ...
                            value="http://10.65.106.26/create_batch_hosting_pay_to_card_trade/controller/notifyurl.php">
                        <br></td>
                ...
                <tr>
                    ...
                    <td style="vertical-align: top; width: 50%"><input
                            ...
                            value="create_batch_hosting_pay_to_card_trade"> <br></td>
                    <td style="vertical-align: top; width: 25%;">接口名称 非空
                        create_single_hosting_pay_to_card_trade 创建单笔代付到提现卡交易
                    </td>
                </tr>
                <tr>
                    <td style="vertical-align: top; width: 25%;">支付请求号： <br>
                    </td>
                    <td style="vertical-align: top; width: 50%"><input
                            style="width: 100%;" id="out_pay_no" name="out_pay_no"
                            value="<?php echo date("YmdHis") ?>"> <br></td>
                    <td style="vertical-align: top; width: 25%;">商户网站支付订单号，商户内部保证唯一</td>
                </tr>
                <tr>
                ...
                <tr>
                    <td style="vertical-align: top; width: 25%;">交易列表： <br>
                    </td>
                    <td style="vertical-align: top; width: 50%"><input
                            style="width: 100%;" id="trade_list"
                            name="trade_list" value=""> <br></td>
                    <td style="vertical-align: top; width: 25%;">
                        无长度范围限制，详见“交易参数”。参数间用“~”分隔，各条目之间用“$”分隔，备注信息不要包含特殊分隔符范例：20131105154925~
                        binding_card^10014543,UID,5446010~300.00~房贷还款清偿~$20131105154926~binding_card^10014543,UID,5446010~300.00~房贷还款清偿~
                    </td>
                </tr>
                <tr>
                    <td style="vertical-align: top; width: 25%;">通知方式： <br>
                    </td>
                    <td style="vertical-align: top; width: 50%">
                        <select id="notify_method" name="notify_method">
                            <option value="single_notify">交易逐笔通知</option>
                            <option value="batch_notify">批量通知</option>
                        </select>
                        <br></td>
                    <td style="vertical-align: top; width: 25%;">通知方式 取值范围：
                        single_notify, batch_notify
...
<script>
    ...
    function checkForm() {
        ...
        //交易列表
        var trade_list = $("#trade_list").val();
        if (typeof (trade_list) != "undefined" && trade_list) {
            params += '&trade_list=' + trade_list;
        } else {
            $("#error").html("请输入交易列表");
            return false;
        }
        //通知方式
        var notify_method = $("#notify_method").val();
        if (typeof (notify_method) != "undefined" && notify_method) {
            params += '&notify_method=' + notify_method;
        } else {
            $("#error").html("请输入通知方式");
            return false;
        }
