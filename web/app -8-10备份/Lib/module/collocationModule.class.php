...
class collocationModule extends SiteBaseModule
{
	...
	public function SetRealName(){
		...
		if (empty($data)){
			...
			if (empty($data['idno'])){
				...
				$collocation_class = $class_name."_collocation";
				$collocation_object = new $collocation_class();
				$collocation_code = $collocation_object->SetRealName($user_id);
				print_r($collocation_code);
			}			
		}
	}
	 ...
	 */
	public function BindingVerify(){
		$user_id = intval(strim($_REQUEST['user_id']));
		$verify_type=strim($_REQUEST['verify_type']);
		$data = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where id = ".$user_id);
		...
		if (empty($data)){
			...
			if (empty($data['mobile'])){
				...
				$collocation_class = $class_name."_collocation";
				$collocation_object = new $collocation_class();
				$collocation_code = $collocation_object->BindingVerify($user_id,$verify_type);
				print_r($collocation_code);
			}			
		}
	}		
	...
	public function get_bank_info()
	{
		...
		if (empty($data)){
			...
			if (!empty($data['ips_acct_no'])){
				...
				$collocation_class = $class_name."_collocation";
				$collocation_object = new $collocation_class();
				$collocation_code = $collocation_object->get_bank_info($user_id);
				print_r($collocation_code);
			}else{
				showErr('该用户还未申请过资金托管帐户',0);
			}
	 ...
	 * 2.7	绑定银行卡推进
	 */
	public function BindBankCardAdvance(){
	    $id = intval(strim($_REQUEST['id']));
	    $user_id = intval(strim($_REQUEST['user_id']));
	    $ticket = strim($_REQUEST['ticket']);
	    $data = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where id = ".$user_id);
	    ...
	    if (empty($data)){
	        ...
	        if (!empty($data['ips_acct_no'])){
	            ...
	            $collocation_class = $class_name."_collocation";
	            $collocation_object = new $collocation_class();
	            $collocation_code = $collocation_object->BindBankCardAdvance($user_id,$ticket,$id);
	            print_r($collocation_code);
	        }else{
	            showErr('该用户还未申请过资金托管帐户',0);
	        }
	...
	/**
	 *解绑银行卡 UnbindingBankCard
	 */
	public function UnbindingBankCard(){
	    $card_id = strim($_REQUEST['card_id']);
	    $user_id = intval(strim($_REQUEST['user_id']));
	    $data = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where id = ".$user_id);
	    if (empty($data)){
	        showErr('用户不存在',0);
	        ...
	        if (!empty($data['ips_acct_no'])){
	            ...
	            $collocation_class = $class_name."_collocation";
	            $collocation_object = new $collocation_class();
	            $collocation_code = $collocation_object->UnbindingBankCard($user_id,$card_id);
	            print_r($collocation_code);
	        }else{
	            showErr('该用户还未申请过资金托管帐户',0);
	        }
		...
		if (empty($data)){
			...
			if (!empty($data['ips_acct_no'])){
				...
				foreach($result as $k=>$v){
					if($k=="pNeedstl" || $k=="pBalance" || $k=="pLock" || $k=="bonus")
						$result[$k] = round($v,2);
				}
				if($is_ajax==1){
					ajax_return($result);
