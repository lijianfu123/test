...
class uc_ipsModule extends SiteBaseModule
{
	...
	{
		$GLOBALS['tmpl']->assign("page_title","我的存钱罐");
		...
		$request["identity_id"]=to_guid_string((int)$GLOBALS["user_info"]["id"]);
		$request["identity_type"]="UID";
		$request["account_type"]="SAVING_POT";
		
		$GLOBALS['tmpl']->assign("sina_id",$request["identity_id"]);
		...
		$result = $weibopay->curlPost("https://gate.pay.sina.com.cn/mgs/gateway.do",$tijiao); // 使用模拟表单提交进行数据提交
		...
		if ($weibopay->checkSignMsg ($splitdata,$sign_type)) {
			if ($splitdata["response_code"] == 'APPLY_SUCCESS') {
				...
				$GLOBALS['tmpl']->assign("zuori",$ff[0]);
				$GLOBALS['tmpl']->assign("zuijinyigeyue",$ff[1]);
				$GLOBALS['tmpl']->assign("zong",$ff[2]);
				$GLOBALS['tmpl']->assign("yue",$splitdata["balance"]);
			}else{
				//print_r($splitdata); 
			}
		} 
		...
		$request=array();
		$request["service"]="query_fund_yield";
		$request["version"]=sinapay_version;
		$request["request_time"]=date("YmdHis");
		$request["partner_id"]=sinapay_partner_id;
		$request["_input_charset"]=sinapay_input_charset;
		$request["fund_code"]="000330";
		$request["sign"]="";
		$request["sign_type"]="MD5";
		ksort($request);//对签名参数据排序
		...
		$tijiao = $weibopay->createcurl_data($request); // 调用createcurl_data创建模拟表单需要的数据
		$result = $weibopay->curlPost("https://gate.pay.sina.com.cn/mas/gateway.do",$tijiao); // 使用模拟表单提交进行数据提交
		$result = urldecode ($result);
		$splitdata = array ();
		$splitdata = json_decode($result,true);
		
		$yield_list=explode("|",$splitdata["yield_list"]);
		foreach ($yield_list as $key=>$val){
			if($key<7){
				$yield_lists=explode("^",$val);
				$dd[$key]["riqi"]=date("m-d",strtotime($yield_lists[0]));
				$dd[$key]["qiri"]=$yield_lists[1];
				$dd[$key]["gao"]=$yield_lists[1]*66.08591168519074;
				$dd[$key]["wanfen"]=$yield_lists[2];
			}
		}
		//print_r($dd);
		$GLOBALS['tmpl']->assign("dd",$dd);
		
		$GLOBALS['tmpl']->assign("user_id",$GLOBALS["user_info"]["id"]);
			$GLOBALS['tmpl']->assign("inc_file","inc/uc/cunqianguan.html");
			$GLOBALS['tmpl']->display("page/uc.html");
	...
	public function cunqianguan1()
	{
		$GLOBALS['tmpl']->assign("page_title","我的账户");
	...
	public function cunqianguanwap()
	{
		$GLOBALS['tmpl']->assign("page_title","我的存钱罐");
		...
		//print_r($dd);
		$GLOBALS['tmpl']->assign("dd",$dd);
		$GLOBALS['tmpl']->display("page/cunqianguanwap.html");
	...
	public function myzh()
	{
		$GLOBALS['tmpl']->assign("page_title","我的zh");
		...
		//print_r($dd);
		$GLOBALS['tmpl']->assign("dd",$dd);
		$GLOBALS['tmpl']->display("page/myzh.html");
	...
	public function recharge()
	{
		...
		elseif(strtolower($className) == "sina")
		{
			$where = " where is_callback = 1 and yr.response_code = 'APPLY_SUCCESS' and yr.user_id = ".$GLOBALS["user_info"]["id"];
			if(strim($start_time)!="")
			{ 
				$where .= " and yr.create_time >=".to_timespan(strim($start_time));
			}
			...
			$list = $GLOBALS['db']->getAll("select out_trade_no as pMerBillNo,idno as pIdentNo,
			real_name as pRealName, FROM_UNIXTIME( yr.create_time, '%Y-%m-%d' ) as pTrdDate,
			amount as pTrdAmt,recharge_type as pChannelType ,yr.deposit_status as status,fee_taken_on as pIpsFeeType ,user_fee as pMerFee from ".DB_PREFIX."sina_recharge yr left join ".DB_PREFIX."user u on yr.user_id = u.id ".$where." order by yr.id DESC".$limit);
			$list_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."sina_recharge yr left join ".DB_PREFIX."user u on yr.user_id = u.id ".$where);
		} 
//,fee_taken_on as pIpsFeeType
	...
	public function transfer()
	{
		...
		elseif(strtolower($className) == "sina")
		{
			//$where = " where yw.response_code ='APPLY_SUCCESS' and yw.is_callback = 1 and yw.user_id = ".$GLOBALS["user_info"]["id"];
		    $where = " where yw.user_id = ".$GLOBALS["user_info"]["id"];
			if(strim($start_time)!="")
			{
				$where .= " and yw.create_time >=".to_timespan(strim($start_time));
			}
			...
			$y_sql = "select out_trade_no as pIpsBillNo,yw.withdraw_status as status,u.real_name as pRealName,
			FROM_UNIXTIME(yw.create_time ,'%Y-%m-%d')as pDwDate,yw.create_time time,amount as pTrdAmt,user_fee as pMerFee , 
			u.idno as pIdentNo,
			fee_taken_on as pIpsFeeType
			 from ".DB_PREFIX."sina_fo_charge yw left join ".DB_PREFIX."user u on yw.user_id = u.id ".$where." order by time DESC".$limit;
			// echo $y_sql;
			$list = $GLOBALS['db']->getAll($y_sql);
			$list_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."sina_fo_charge yw left join ".DB_PREFIX."user u on yw.user_id = u.id ".$where);
		}
	...
	public function creditor()
	{
		...
		elseif(strtolower($className) == "sina")
		{
			$where = " where action_type = '1' and  trade_status ='TRADE_FINISHED' and is_callback = 1 and a.load_user_id = ".$GLOBALS["user_info"]["id"];
			if(strim($start_time)!="")
			{
				$where .= " and (a.create_time/1000) >=".to_timespan(strim($start_time));
			}
			if(strim($end_time) !="")
			{
				$where .= " and (a.create_time/1000) <=".  to_timespan(strim($end_time));
			}
			$list = $GLOBALS['db']->getAll("select b.name as deal_name,
			a.goods_id as pBidNo,load_amount as pTrdAmt, 
			user_fee as pFee, load_amount as pTransferAmt,
			FROM_UNIXTIME(a.create_time ,'%Y-%m-%d')as pIpsTime,
			1 as pRegType
			 from ".DB_PREFIX."sina_business as a left join ".DB_PREFIX."deal as b on a.goods_id = b.id ".$where." order by a.id desc ".$limit);
			$list_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."sina_business as a left join ".DB_PREFIX."deal as b on a.goods_id = b.id ".$where);
	...
	public function repayment()
	{
		...
		elseif(strtolower($className) == "sina")
		{
			$condition = " where yct.action_type = 4 and yct.trade_status ='FINISHED' and yct.is_callback = 1 and yct.brw_id = ".$GLOBALS["user_info"]["id"];
			
			$sql = "select yct.goods_id as pBidNo, yct.*,yct.id as mid,d.`name` as deal_name, 1 as pRepayType,true_repay_money as pOutAmt,
			u.user_name, dr.* , FROM_UNIXTIME(yct.create_time ,'%Y-%m-%d')as pRepaymentDate
			from ".DB_PREFIX."sina_business as yct
			left join ".DB_PREFIX."deal d on d.id = yct.goods_id
			left join ".DB_PREFIX."deal_repay dr on dr.id = yct.deal_repay_id 
			left join ".DB_PREFIX."user u on u.id = d.user_id ";
			
			$count_sql = "select count(*) from ".DB_PREFIX."sina_business as yct
			left join ".DB_PREFIX."deal d on d.id = yct.goods_id
			left join ".DB_PREFIX."deal_repay dr on dr.id = yct.deal_repay_id 
			left join ".DB_PREFIX."user u on u.id = d.user_id ";
			...
			//echo $sql.$condition." order by mid desc ".$limit;
			$list = $GLOBALS['db']->getAll($sql.$condition." order by mid desc ".$limit);
			$list_count = $GLOBALS['db']->getOne($count_sql.$condition);
		}
	...
	public function repayment_view()
	{
		...
		elseif(strtolower($className) == "sina")
		{
			...
			$sql = "select d.*,r.*,d.id as mid ,u.user_name,dl.name as deal_name,tu.user_name as t_user_name 
			from ".DB_PREFIX."sina_business_detail as d
			left JOIN ".DB_PREFIX."deal_load_repay r on r.id = d.deal_load_repay_id
			left join ".DB_PREFIX."user u on u.id = r.user_id
			left join ".DB_PREFIX."user tu on tu.id = r.t_user_id left join ".DB_PREFIX."deal as dl on r.deal_id = dl.id";
	...
	public function receivedpayment()
	{
		...
		elseif(strtolower($className) == "sina")
		{
			...
			$sql = "select d.*,r.*,deal_t.goods_id as pBidNo,d.id as mid ,u.user_name,dl.name as deal_name 
			from ".DB_PREFIX."sina_business_detail as d
			left JOIN ".DB_PREFIX."deal_load_repay r on r.id = d.deal_load_repay_id
			left JOIN ".DB_PREFIX."deal dl on dl.id = r.deal_id
			left join ".DB_PREFIX."user u on u.id = dl.user_id
			left JOIN ".DB_PREFIX."sina_business deal_t on deal_t.id = d.pid";
			...
			left JOIN ".DB_PREFIX."deal dl on dl.id = r.deal_id
			left join ".DB_PREFIX."user u on u.id = dl.user_id";
			//echo  $sql.$condition." order by d.id desc".$limit;
			$list = $GLOBALS['db']->getAll( $sql.$condition." order by d.id desc".$limit);
...
//新浪
		elseif(strtolower($className) == "sina")
		{
			$condition = " where  action_type = '2' and t.response_code ='APPLY_SUCCESS' and is_callback = 1 and t.user_id = ".$GLOBALS["user_info"]["id"];
			
			if(strim($start_time)!="")
			{
				$condition .= " and (t.request_time/1000) >=".to_timespan(strim($start_time));
			}
			if(strim($end_time) !="")
			{
				$condition .= " and (t.request_time/1000) <=".  to_timespan(strim($end_time));
			}
			
			$sql = "select d.id,d.`name`,u.user_name,d.borrow_amount as borrow_amount,d.id as pBidNo,
			d.borrow_amount*CONVERT(d.services_fee,DECIMAL)*0.01 as deal_fee,
			(d.borrow_amount - (d.borrow_amount*CONVERT(d.services_fee,DECIMAL)*0.01)) as loan_amount,
			1 as pTransferMode, out_trade_no as pIpsBillNo, FROM_UNIXTIME(t.create_time ,'%Y-%m-%d') as pIpsTime
			 from ".DB_PREFIX."sina_business as t
			LEFT JOIN ".DB_PREFIX."deal as d on d.id = t.goods_id
			LEFT JOIN ".DB_PREFIX."user as u on u.id = d.user_id ";
			
			$count_sql = "select count(*) from ".DB_PREFIX."sina_business as t
			LEFT JOIN ".DB_PREFIX."deal as d on d.id = t.goods_id
			LEFT JOIN ".DB_PREFIX."user as u on u.id = d.user_id ";
			
			//print_r($sql.$condition);die;
			//取得满足条件的记录数
			$list = $GLOBALS['db']->getAll( $sql.$condition."  group by t.goods_id order by id desc ".$limit);
			//echo  $sql.$condition."  group by t.goods_id order by id desc ".$limit;
			$list_count = $GLOBALS['db']->getOne($count_sql.$condition);
		}
		$page = new Page($list_count,app_conf("PAGE_SIZE"));   //初始化分页对象 		
		$p  =  $page->show();
	...
	LEFT JOIN ".DB_PREFIX."deal_load l on  l.pMerBillNo = t.order_id ";
		...
		//新浪DH 20160802添加满标放款详细信息
		elseif(strtolower($className) == "sina"){
			$list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal_load where deal_id = ".intval(strim($_REQUEST['id']))." and is_has_loans = 1" );
			foreach($list as $key=>$val){
				$list[$key]['pIpsDetailBillNo'] = $val['pMerBillNo'];
				$list[$key]['pTrdAmt'] = $val['money'];
			}
			$list_count = count($list);
//
//			$condition = " where  action_type = '2' and t.response_code ='APPLY_SUCCESS' and is_callback = 1 ";
//
//			$condition .= " and t.goods_id = ".intval(strim($_REQUEST['id']));
//
//			$GLOBALS['tmpl']->assign('id',intval(strim($_REQUEST['id'])));
//
//			$sql = "select t.id as mid,l.*,
//			l.pMerBillNo as pIpsDetailBillNo,l.money as pTrdAmt,t.user_fee as pIpsFee
//			 from ".DB_PREFIX."sina_business as t
//	LEFT JOIN ".DB_PREFIX."deal_load l on  l.pMerBillNo = t.out_trade_no ";
//
//			$count_sql = "select count(*) from ".DB_PREFIX."sina_business as t
//	LEFT JOIN ".DB_PREFIX."deal_load l on  l.pMerBillNo = t.out_trade_no ";
//
